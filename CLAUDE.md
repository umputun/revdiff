...
- `app/ui/` - bubbletea TUI package. All files share one `Model` struct — methods are split across files by concern to keep code files under ~500 lines and test files around ~1000 lines (soft target). Each source file has a matching `_test.go` file. See `app/ui/doc.go` for package-level documentation.
  ...
  - `handlers.go` - modal handlers (enter/esc, discard, filter, mark reviewed), help spec building
...
- `app/ui/overlay/` - layered popup UI sub-package. Owns help, annotation list, and theme selector overlays — all popup state, rendering, key dispatch, and ANSI-aware centered compositing. Manager coordinator enforces mutual exclusivity (one overlay at a time). Four main files:
  - `overlay.go` - Manager struct (coordinator), Kind/OutcomeKind enums, Outcome, RenderCtx, Resolver interface, overlayCenter, injectBorderTitle
  - `help.go` - helpOverlay type: two-column help popup rendering, key handling
  - `annotlist.go` - annotListOverlay type: annotation list popup rendering, item formatting, navigation, key handling
  - `themeselect.go` - themeSelectOverlay type: theme selector popup with fzf-style filter, swatch preview, key handling
...
- `Renderer` - `ChangedFiles()`, `FileDiff()` - implemented by `diff.Git`, `diff.Hg`, `diff.FallbackRenderer`, `diff.FileReader`, `diff.DirectoryReader`, `diff.StdinReader`, `diff.ExcludeFilter`, `diff.IncludeFilter`
...
- `overlayManager` - `Active()`, `Kind()`, `OpenHelp()`, `OpenAnnotList()`, `OpenThemeSelect()`, `Close()`, `HandleKey()`, `Compose()` - implemented by `*overlay.Manager`. Injected via `ModelConfig.Overlay` wired in `app/main.go`
...
- ~30 `Action` constants in `app/keymap/keymap.go` (e.g., `ActionDown`, `ActionQuit`); modal text-entry keys (annotation input, search input, confirm discard) stay hardcoded; overlay key dispatch in `app/ui/overlay/` uses keymap actions for j/k/up/down but keeps `enter` and `esc` hardcoded
- Help overlay is dynamically rendered from `m.keymap.HelpSections()` via `overlay.HelpSpec`
...
- Overlay popups (help, annotation list, theme selector) are managed by `overlay.Manager` in `app/ui/overlay/`. `Manager.Compose()` uses `overlayCenter()` (ANSI-aware compositing via `charmbracelet/x/ansi.Cut`) to render the active popup on top of existing content; background (tree pane) remains visible at the edges. `Manager.HandleKey()` returns an `Outcome` value — Model switches on `OutcomeKind` to perform side effects (file jumps, theme apply/persist)
...
- Annotation list popup (`@` key): rendering and navigation live in `app/ui/overlay/annotlist.go`; spec building and cross-file jump logic stay in `app/ui/annotlist.go`. Cross-file jumps use `pendingAnnotJump` field: stores target annotation, triggers file load via `selectByPath`, then `handleFileLoaded` checks and positions cursor. Guard: `pendingAnnotJump.File == msg.file` prevents stale jumps.
