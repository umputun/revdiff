...
- `app/ui/` - bubbletea TUI package. All files share one `Model` struct — methods are split across files by concern to keep code files under ~500 lines and test files around ~1000 lines (soft target). Each source file has a matching `_test.go` file. See `app/ui/doc.go` for package-level documentation.
  ...
  - `worddiff.go` - intra-line word-diff highlighting (`W` toggle): pairs remove/add lines per hunk via `pairHunkLines`, computes byte ranges via `computeIntraRanges` (diffmatchpatch + semantic cleanup), overlays bg via `insertBgMarkers`
...
- Status bar mode icons (`▼ ◉ ↩ ≋ ⊟ ⇄ # @`) are always rendered on the right side via `statusModeIcons()`. `@` indicates blame gutter active via `B` toggle. `⊟` indicates tree/TOC pane hidden via `t` toggle. `⇄` indicates word-diff intra-line highlighting active via `W` toggle. Active modes use `StatusFg`, inactive use `Muted` — both via raw ANSI fg sequences. Graceful degradation on narrow terminals drops left segments: search position first (`statusSegmentsNoSearch`), then line number and hunk info (`statusSegmentsMinimal`), then truncates filename.
