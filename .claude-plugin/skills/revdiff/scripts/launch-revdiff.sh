...
APPLESCRIPT
    ...
    # `|| true` because the window may already be gone by the time we
    # try to close it (user dismissed the "press any key" prompt manually,
    # or Ghostty cleared it on its own). without this, set -e would exit
    # before `cat "$OUTPUT_FILE"` and the annotations would be lost.
    osascript - "$GHOSTTY_KIND" "$GHOSTTY_TID" <<'APPLESCRIPT' 2>/dev/null || true
on run argv
