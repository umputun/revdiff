...
**Tagging the review session (tmux)**: the tmux backend runs each review in a transient detached session. If other tooling reacts to tmux sessions (status bars, session managers, dashboards), set `REVDIFF_TMUX_SESSION_OPTIONS` to whitespace-separated `key=value` tokens; the launcher applies each as a tmux session option before any client attaches (e.g. `REVDIFF_TMUX_SESSION_OPTIONS="@my-manager-ignore=1"` lets a session manager recognize and skip the transient session). Values cannot contain spaces. This variable configures the launcher itself — set it in the shell environment where the plugin runs; it is unrelated to revdiff's own env vars, which don't survive into the overlay. The launcher sets three options of its own — `@revdiff_title`, `@revdiff_output`, and `@revdiff_sentinel` — which are what `--resume` reads to find a backgrounded review.

**Resuming a backgrounded review (tmux)**: dismissing the popup (`prefix d`) leaves the review running in its session. `launch-revdiff.sh --resume` re-opens the most recent backgrounded review in a fresh popup, waits on it exactly like the original launch did, and prints the annotations on the same exit codes (`0` clean, `10` annotations captured). Because the resume metadata lives on the tmux session rather than in the launcher process, this still works after the original launcher is gone — the normal case once an AI harness kills it at its command-timeout cap. `--resume` takes no other arguments and must run inside tmux; with no backgrounded review it exits 1 with `no backgrounded revdiff review found`.
