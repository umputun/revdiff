...
The override receives the same positional arguments the bundled launcher does (`[base] [against] [--staged] [--only=file1] [--all-files] [--exclude=prefix] [--description=text|--description-file=path]`). The bundled launcher appends `--exit-code-on-annotations` internally; custom launchers should do the same, print captured annotations to stdout, and preserve exit `10` as success-with-annotations. Other nonzero statuses remain launcher failures.
