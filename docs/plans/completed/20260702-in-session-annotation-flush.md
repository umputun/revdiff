...
- Persistence lives in the annotation domain: `annotation.Store` already owns serialization (`FormatOutput`/`Load`), so it also owns writing itself to a file — `(s *Store) WriteFile(path string) error` writes atomically (replacing the exit path's bare `os.WriteFile`, a truncate-then-write partial-read hazard against a concurrent AI reader). No new package, no interface, no injected dependency — the Model calls the store directly.

> **Post-review adjustment (PR #249):** `Store.WriteFile` delegates to the pre-existing `fsutil.AtomicWriteFile` (temp file + rename) rather than a hand-rolled copy, and the exit-time `writeAnnotationOutput` file branch calls `fsutil.AtomicWriteFile` directly with the already-computed output string (single source for file content, exit code, and stdout) rather than routing through `Store.WriteFile`. Both write paths still share one atomic writer. The detailed Technical Details / Task 5 sections below describe the original store-routed approach.
...
## Solution Overview
- `annotation.Store` gains `WriteFile(path string) error`: it formats itself via `FormatOutput()` and writes atomically (temp file + rename, mode 0o600). Atomic on a single filesystem, so a concurrent reader sees either the old or the new complete file, never a truncated one. Per the post-review note above, the atomic write is `fsutil.AtomicWriteFile` (the repo's existing helper), not hand-rolled.
...
- `app/main.go` sets `ModelConfig.OutputPath: opts.Output`; the exit-time `writeAnnotationOutput` file branch writes atomically instead of via `os.WriteFile` (final impl calls `fsutil.AtomicWriteFile` directly with the already-computed output string — see post-review note in Overview). The stdout branch, empty-output skip, and exit-code logic stay at the composition root.
