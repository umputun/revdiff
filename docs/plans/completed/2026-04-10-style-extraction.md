...
- **Decision**: every source file in `app/ui/style/` has a matching `_test.go`: `style_test.go`, `color_test.go`, `resolver_test.go`, `renderer_test.go`, `sgr_test.go`. Plus the enum exhaustiveness tests inside `resolver_test.go`.
- **Why**: the old branch shipped `ansi.go` with no test file — that's the specific gap this decision exists to prevent. Every non-trivial file in `style/` gets tests, no exceptions.
- **Note on file consolidation**: an early draft of this plan scoped a separate `ansi.go` + `ansi_test.go`. During implementation those were merged into `color.go` + `color_test.go` because `Write`, `ansiColor`, `hexVal`, and the HSL math are all tightly-coupled ANSI-output machinery that belongs next to the `Color` type. The rule "every file has its test file" still holds — just for 5 files instead of 6. Coverage for `Write`/`ansiColor`/`hexVal` (the content D13 was most worried about) lives in `color_test.go`.
...
├── color.go              — Color type + ResetFg/ResetBg consts + Write helper + private ansiColor primitive + private hexVal helper + private HSL helpers (shiftLightness, parseHexRGB, rgbToHSL, hslToRGB, hueToRGB). All ANSI-output machinery lives here alongside the Color type — they're tightly related.
├── resolver.go           — Resolver type + 6 methods (Color, Style, LineBg, LineStyle, WordDiffBg, IndicatorBg) + private newResolver/plainResolver constructors + private lipgloss builders (contextStyle, dirEntryStyle, etc., as methods on Colors)
...
├── style.go              — Colors struct + (c Colors) normalize() method (no constructors here — each type owns its own constructor)
├── color_test.go         — Color type + reset consts + HSL math + Write/ansiColor/hexVal tests (Write_EmptyColor, Write_NonEmptyColor, Write_WriterError, TestAnsiColor, TestHexVal)
...
└── style_test.go         — Colors normalize() (plus construction tests)
```

**Note on the file layout**: an early draft of this plan split `color.go` and `ansi.go` as separate files. During implementation they were consolidated into a single `color.go` because `Write`, `ansiColor`, `hexVal`, `HSL` math, and the `Color` type itself are all tightly-related ANSI-output machinery — splitting them across two files fragmented a single conceptual unit. The rule "one test file per source file" still holds: `color.go` has `color_test.go` covering all of its contents including the `Write`/`ansiColor`/`hexVal` tests that the plan originally scoped to `ansi_test.go`. Test coverage requirement from D13 ("`ansi.go` on the old branch had zero tests — fix that") is met; the file name changed but the content and coverage are intact.
...
- `style_test.go` — `(c Colors) normalize()` method (hex normalization, WordAddBg/WordRemoveBg auto-derivation via shiftLightness), `Colors` zero-value behavior. Small file because most of style.go is just the `Colors` struct declaration plus the `normalize` method.
- `color_test.go` — covers everything in `color.go`, which is a lot:
  - **ANSI output primitives** (previously scoped to a separate `ansi_test.go` file in an earlier plan draft): `Write` (empty color, non-empty color, plain text, writer error propagation), `ansiColor` (fg/bg codes, invalid hex returns empty), `hexVal` (all hex digits 0-9/a-f/A-F, invalid chars return 0). **This is critical because the old branch shipped `ansi.go` without any test file — coverage for these functions now exists as `TestWrite_*`, `TestAnsiColor`, and `TestHexVal` in `color_test.go`.**
  - **Color type + reset constants**: `TestResetConstants` verifies `ResetFg`/`ResetBg` literal values. No methods on `Color` itself to test per D3.
  - **HSL math**: `shiftLightness` (dark→lighter, light→darker, saturation 0, black/white edge cases, boundary clamp), `parseHexRGB` (valid/invalid), `rgbToHSL`/`hslToRGB` (roundtrip + known values), `hueToRGB` (all 4 branches, exact boundary values).
...
#### Task 1.4: Add ANSI output primitives (Write, ansiColor, hexVal) to `color.go`

**Files:**
- Modify: `app/ui/style/color.go` (add ANSI primitives to the existing file from Task 1.3)
- Modify: `app/ui/style/color_test.go` (add ANSI primitive tests to the existing file from Task 1.3)

**Note on file layout**: an early draft of this plan scoped a separate `ansi.go` + `ansi_test.go` for these helpers. During implementation they were consolidated into `color.go` / `color_test.go` because `Write`, `ansiColor`, `hexVal`, and the `Color` type itself are all tightly-related ANSI-output machinery — splitting them across two files fragmented a conceptual unit. Task 1.4 is retained as a separate task to preserve the implementation steps, but it's now about **adding content to color.go** rather than creating a new file.

- [x] add `func Write(w io.Writer, c Color) (int, error)` to `color.go` (standalone per D10)
...
- [x] add tests to `color_test.go` for `Write` — empty color (0 bytes, nil err), non-empty color (writes exact bytes), plain text pass-through, writer error propagation (use a fake writer)
- [x] add tests for `ansiColor` — valid 7-char hex with `#`, valid 6-char without `#`, invalid inputs return empty
- [x] add tests for `hexVal` — all hex digits 0-9, a-f, A-F, invalid chars return 0
- [x] **this was the set of functions the old branch skipped (shipped `ansi.go` with zero test file) — full coverage is a hard requirement, and it's met inside `color_test.go`**
...
- [x] implement `(r Resolver) Color(k ColorKey) Color` with a switch over all 11 keys. Each returns the ANSI fg or bg sequence via private `ansiColor` helper (in `color.go`). `ColorKeyStatusFg` case implements `StatusFg || MutedFg` fallback.
