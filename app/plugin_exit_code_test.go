...
func TestPiCallerPreservesAnnotationExitCode(t *testing.T) {
	...
	assert.Contains(t, src, `const EXIT_CODE_ON_ANNOTATIONS_FLAG = "--exit-code-on-annotations";`)
	assert.Contains(t, src, "const commandArgs = [...withAnnotationExitCode(launch.args), `--output=${outputFile}`];")
	assert.Contains(t, src, "spawnSync(launcher, withAnnotationExitCode(launch.args)")
