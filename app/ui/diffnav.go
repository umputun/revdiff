...
func (m Model) handleTreeNav(action keymap.Action) (tea.Model, tea.Cmd) {
	...
	case keymap.ActionScrollCenter:
		m.tree.ScrollToCenter(m.treePageSize())
		return m, nil
	case keymap.ActionScrollTop:
		m.tree.ScrollToTop()
		return m, nil
	case keymap.ActionScrollBottom:
		m.tree.ScrollToBottom(m.treePageSize())
		return m, nil
...
func (m Model) handleTOCNav(action keymap.Action) (tea.Model, tea.Cmd) {
	...
	case keymap.ActionScrollCenter:
		m.file.mdTOC.ScrollToCenter(m.treePageSize())
		return m, nil
	case keymap.ActionScrollTop:
		m.file.mdTOC.ScrollToTop()
		return m, nil
	case keymap.ActionScrollBottom:
		m.file.mdTOC.ScrollToBottom(m.treePageSize())
		return m, nil
	case keymap.ActionFocusDiff, keymap.ActionScrollRight:
		if m.file.name != "" {
			m.layout.focus = paneDiff
