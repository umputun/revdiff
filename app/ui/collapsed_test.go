...
func TestModel_FileSwitchResetsExpandedHunksPreservesCollapsed(t *testing.T) {
	...
	assert.True(t, model.modes.collapsed.enabled, "collapsed should persist across file switches")
	assert.Empty(t, model.modes.collapsed.expandedHunks, "expandedHunks should be reset on file switch")
...
func TestModel_CollapsedDeleteOnlyPlaceholderBlocksAnnotation(t *testing.T) {
	...
	assert.False(t, m.annot.annotating, "annotating mode should not be active")
}
...
func TestModel_CollapsedCursorMovementIncludesDeleteOnlyPlaceholder(t *testing.T) {
	...
	assert.Equal(t, 1, m.nav.diffCursor, "should land on delete-only hunk placeholder")

	// move down again should skip hidden idx 2 and land on ctx2 (idx 3)
	m.moveDiffCursorDown()
	assert.Equal(t, 3, m.nav.diffCursor, "should skip hidden remove and land on context")

	// move up should go back to placeholder
	m.moveDiffCursorUp()
	assert.Equal(t, 1, m.nav.diffCursor, "should go back to placeholder")
}
...
func TestModel_CollapsedCursorDownSkipsRemovedLines(t *testing.T) {
	...
	model := result.(Model)
	model.modes.collapsed.enabled = true
	assert.Equal(t, 0, model.nav.diffCursor, "starts on ctx1")

	// move down should skip removed lines (indices 1,2) and land on add line (index 3)
	model.moveDiffCursorDown()
	assert.Equal(t, 3, model.nav.diffCursor, "should skip removed lines and land on add line")

	// move down again lands on ctx2
	model.moveDiffCursorDown()
	assert.Equal(t, 4, model.nav.diffCursor, "should land on ctx2")
}
...
func TestModel_CollapsedCursorUpSkipsRemovedLines(t *testing.T) {
	...
	model := result.(Model)
	model.modes.collapsed.enabled = true
	model.nav.diffCursor = 4 // start on ctx2

	// move up should skip removed lines (indices 2,1) and land on add line (index 3)
	model.moveDiffCursorUp()
	assert.Equal(t, 3, model.nav.diffCursor, "should land on add line")

	// move up again skips removed lines and lands on ctx1
	model.moveDiffCursorUp()
	assert.Equal(t, 0, model.nav.diffCursor, "should skip removed lines and land on ctx1")
}
...
func TestModel_CollapsedCursorMovementInExpandedHunk(t *testing.T) {
	...
	model := result.(Model)
	model.modes.collapsed.enabled = true
	model.modes.collapsed.expandedHunks = map[int]bool{1: true} // expand the hunk starting at index 1

	// cursor on ctx1, move down should land on removed line since hunk is expanded
	model.moveDiffCursorDown()
	assert.Equal(t, 1, model.nav.diffCursor, "should land on removed line in expanded hunk")

	// move down lands on add line
	model.moveDiffCursorDown()
	assert.Equal(t, 2, model.nav.diffCursor, "should land on add line")

	// move down lands on ctx2
	model.moveDiffCursorDown()
	assert.Equal(t, 3, model.nav.diffCursor, "should land on ctx2")
	...
	model.moveDiffCursorUp()
	assert.Equal(t, 0, model.nav.diffCursor, "should land on ctx1")
}
...
func TestModel_CollapsedSkipInitialDividers(t *testing.T) {
	t.Run("skips divider and removed lines", func(t *testing.T) {
		...
		assert.Equal(t, 2, m.nav.diffCursor, "should skip divider and removed line, land on add")
	})
	...
	t.Run("expanded mode skips only dividers", func(t *testing.T) {
		...
		m.file.lines = lines
		m.skipInitialDividers()
		assert.Equal(t, 1, m.nav.diffCursor, "expanded mode should land on removed line after divider")
	})
	...
	t.Run("collapsed with expanded hunk allows removed lines", func(t *testing.T) {
		...
		m.file.lines = lines
		m.skipInitialDividers()
		assert.Equal(t, 1, m.nav.diffCursor, "expanded hunk should allow landing on removed line")
	})
}
...
func TestModel_CollapsedCursorDownMultipleHunks(t *testing.T) {
	...
	positions := []int{model.nav.diffCursor}
	for range 10 {
		prev := model.nav.diffCursor
		model.moveDiffCursorDown()
		if model.nav.diffCursor == prev {
			break
		}
		positions = append(positions, model.nav.diffCursor)
	}
...
func TestModel_CollapsedPageDownSkipsRemovedLines(t *testing.T) {
	...
	assert.Greater(t, model.nav.diffCursor, startCursor, "cursor should advance")
...
func TestModel_CollapsedPageUpSkipsRemovedLines(t *testing.T) {
	...
	model.nav.diffCursor = len(lines) - 1
	startY := model.cursorViewportY()
...
func TestModel_CollapsedCursorToEndSkipsRemovedLines(t *testing.T) {
	...
	assert.Equal(t, 2, m.nav.diffCursor, "should land on add line, not hidden removed lines")
}
...
func TestModel_CollapsedHunkNavigationSkipsRemovedLines(t *testing.T) {
	...
	m.moveToNextHunk()
	assert.Equal(t, 3, m.nav.diffCursor, "should land on first visible line in hunk 1")

	m.moveToNextHunk()
	assert.Equal(t, 6, m.nav.diffCursor, "should land on first visible line in hunk 2")
...
func TestModel_CollapsedHunkNavigationExpandedHunk(t *testing.T) {
	...
	m.moveToNextHunk()
	assert.Equal(t, 1, m.nav.diffCursor, "expanded hunk should land on remove line")
}
...
func TestModel_CollapsedHunkNavigationDeleteOnly(t *testing.T) {
	...
	m.moveToNextHunk()
	assert.Equal(t, 5, m.nav.diffCursor, "should land on mixed hunk's add line")

	// prev hunk from hunk 2 goes back to delete-only hunk 1's placeholder
	m.moveToPrevHunk()
	assert.Equal(t, 1, m.nav.diffCursor, "should go back to delete-only hunk placeholder")
}
...
func TestModel_AdjustCursorIfHidden(t *testing.T) {
	...
	t.Run("cursor on hidden line moves forward", func(t *testing.T) {
		...
		m.file.lines = lines
		m.modes.collapsed.enabled = true
		m.nav.diffCursor = 1 // on hidden removed line
		m.adjustCursorIfHidden()
		assert.Equal(t, 3, m.nav.diffCursor, "should move forward to add line")
	})
	...
	t.Run("cursor on visible line stays put", func(t *testing.T) {
		...
		m.nav.diffCursor = 0 // on context line
		m.adjustCursorIfHidden()
		assert.Equal(t, 0, m.nav.diffCursor, "should stay on context line")
	})
	...
	t.Run("not collapsed mode is no-op", func(t *testing.T) {
		...
		m.nav.diffCursor = 1
		m.adjustCursorIfHidden()
		assert.Equal(t, 1, m.nav.diffCursor, "should not adjust in expanded mode")
	})
	...
	t.Run("cursor on hidden line moves backward to placeholder", func(t *testing.T) {
		...
		m.modes.collapsed.enabled = true
		m.nav.diffCursor = 2 // on hidden removed line (not placeholder)
		m.adjustCursorIfHidden()
		assert.Equal(t, 1, m.nav.diffCursor, "should move backward to delete-only hunk placeholder")
	})
	...
	t.Run("cursor on delete-only hunk placeholder stays put", func(t *testing.T) {
		...
		m.modes.collapsed.enabled = true
		m.nav.diffCursor = 1 // on placeholder (not hidden)
		m.adjustCursorIfHidden()
		assert.Equal(t, 1, m.nav.diffCursor, "placeholder line is visible, cursor should stay")
	})
	...
	t.Run("single hunk all removes placeholder at start", func(t *testing.T) {
		...
		m.modes.collapsed.enabled = true
		m.nav.diffCursor = 0 // on placeholder, not hidden
		m.adjustCursorIfHidden()
		assert.Equal(t, 0, m.nav.diffCursor, "placeholder is visible, cursor stays")
	})
}
...
func TestModel_ToggleCollapsedModeAdjustsCursor(t *testing.T) {
	...
	m.file.name = "a.go"
	m.file.lines = lines
	m.nav.diffCursor = 1 // on removed line

	// toggle to collapsed mode
	m.toggleCollapsedMode()
	assert.True(t, m.modes.collapsed.enabled)
	assert.Equal(t, 2, m.nav.diffCursor, "cursor should move to add line, not stay on hidden removed line")
}
...
func TestModel_ToggleHunkExpansionAdjustsCursor(t *testing.T) {
	...
	m.file.lines = lines
	m.modes.collapsed.enabled = true
	m.modes.collapsed.expandedHunks = map[int]bool{1: true} // hunk expanded
	m.nav.diffCursor = 1                                    // on removed line (visible because expanded)
...
func TestModel_CollapsedCursorDownSkipsPlaceholderAnnotation(t *testing.T) {
	...
	assert.False(t, m.annot.cursorOnAnnotation, "should not stop on invisible annotation of placeholder")
...
func TestModel_CollapsedCursorUpSkipsPlaceholderAnnotation(t *testing.T) {
	...
	m.moveDiffCursorUp()
	assert.Equal(t, 1, m.nav.diffCursor)
	assert.False(t, m.annot.cursorOnAnnotation, "should not land on invisible annotation of placeholder")
}
...
func TestModel_CollapsedToggleClearsAnnotationState(t *testing.T) {
	...
	m.nav.diffCursor = 1
	m.annot.cursorOnAnnotation = true // simulating cursor on annotation in expanded mode

	m.toggleCollapsedMode()
	assert.True(t, m.modes.collapsed.enabled)
	assert.False(t, m.annot.cursorOnAnnotation, "cursorOnAnnotation should be cleared when toggling mode")
}
...
func TestModel_CollapsedHunkCollapseClearsAnnotationState(t *testing.T) {
	...
	m.modes.collapsed.expandedHunks = map[int]bool{1: true}
	m.nav.diffCursor = 1
	m.annot.cursorOnAnnotation = true // on annotation of expanded remove line

	m.toggleHunkExpansion()
	assert.False(t, m.annot.cursorOnAnnotation, "cursorOnAnnotation should be cleared when hunk collapses")
}
