...
func TestHandleDiffNav_WrapperDelegatesToAction(t *testing.T) {
	// sanity check: the thin wrapper produces the same result as the action core.
	// calling handleDiffNav(msg) must equal calling handleDiffAction(Resolve(msg), msg).
	lines := []diff.DiffLine{
		{NewNum: 1, Content: "line1", ChangeType: diff.ChangeContext},
		{NewNum: 2, Content: "line2", ChangeType: diff.ChangeContext},
		{NewNum: 3, Content: "line3", ChangeType: diff.ChangeContext},
	...
	resultNav, _ := m.handleDiffNav(msg)
	modelNav := resultNav.(Model)

	resultAction, _ := m.handleDiffAction(keymap.ActionDown, msg)
	modelAction := resultAction.(Model)

	assert.Equal(t, modelAction.nav.diffCursor, modelNav.nav.diffCursor,
		"wrapper and action core must produce identical cursor state")
}

func TestHandleTreeNav_WrapperDelegatesToAction(t *testing.T) {
	...
	msg := tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'j'}}
	resultNav, _ := m.handleTreeNav(msg)
	modelNav := resultNav.(Model)

	// rebuild fresh model for isolated action-core call
	m2 := testModel([]string{"a.go", "b.go"}, nil)
	m2.tree = testNewFileTree([]string{"a.go", "b.go"})
	m2.layout.focus = paneTree

	resultAction, _ := m2.handleTreeAction(keymap.ActionDown, msg)
	modelAction := resultAction.(Model)

	assert.Equal(t, modelAction.tree.SelectedFile(), modelNav.tree.SelectedFile(),
		"wrapper and action core must produce identical tree selection")
}
