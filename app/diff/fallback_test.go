...
func TestFallbackRenderer_FileDiff_FileHasDiff(t *testing.T) {
	...
	lines, err := fr.FileDiff(FileDiffRequest{Path: "main.go"})
	require.NoError(t, err)
...
func TestFallbackRenderer_FileDiff_NoDiffFallbackToDisk(t *testing.T) {
	...
	fr := NewFallbackRenderer(g, []string{"readme.md"}, dir)
	lines, err := fr.FileDiff(FileDiffRequest{Path: "readme.md"})
	require.NoError(t, err)
	require.Len(t, lines, 3)
...
func TestFallbackRenderer_FileDiff_FileNotInOnly(t *testing.T) {
	...
	lines, err := fr.FileDiff(FileDiffRequest{Path: "hello.go"})
	require.NoError(t, err)
...
func TestFallbackRenderer_FileDiff_FileNotOnDisk(t *testing.T) {
	...
	lines, err := fr.FileDiff(FileDiffRequest{Path: "missing.go"})
	require.NoError(t, err)
...
func TestFallbackRenderer_FileDiff_OutsideRepoAbsPath(t *testing.T) {
	...
	lines, err := fr.FileDiff(FileDiffRequest{Path: tmpFile})
	require.NoError(t, err)
	require.Len(t, lines, 2)
	assert.Equal(t, ChangeContext, lines[0].ChangeType)
...
func TestFallbackRenderer_FileDiff_OutsideRepoNonexistent(t *testing.T) {
	...
	_, err := fr.FileDiff(FileDiffRequest{Path: missingFile})
	require.Error(t, err)
...
func TestFallbackRenderer_FileDiff_InnerErrorWithOnlyFile(t *testing.T) {
	...
	_, err := fr.FileDiff(FileDiffRequest{Ref: "nonexistent-ref-abc123", Path: "readme.md"})
...
func TestFileReader_FileDiff(t *testing.T) {
	...
	lines, err := r.FileDiff(FileDiffRequest{Path: filepath.Join(dir, "readme.md")})
	require.NoError(t, err)
	require.Len(t, lines, 3)
...
func TestFileReader_FileDiff_RelativePath(t *testing.T) {
	...
	lines, err := r.FileDiff(FileDiffRequest{Path: "doc.txt"})
	require.NoError(t, err)
	require.Len(t, lines, 2)
	assert.Equal(t, "alpha", lines[0].Content)
	assert.Equal(t, "beta", lines[1].Content)
...
func TestFileReader_FileDiff_NonexistentFile(t *testing.T) {
	...
	_, err := r.FileDiff(FileDiffRequest{Path: "missing.go"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "stat file")
}
...
func TestFileReader_FullPipeline(t *testing.T) {
	...
	lines, err := r.FileDiff(FileDiffRequest{Path: files[0].Path})
	require.NoError(t, err)
...
func TestStdinReader(t *testing.T) {
	...
	got, err := r.FileDiff(FileDiffRequest{Path: "plan.md"})
	require.NoError(t, err)
	assert.Equal(t, lines, got)

	// wrong filename returns nil
	got, err = r.FileDiff(FileDiffRequest{Path: "other.md"})
	require.NoError(t, err)
	assert.Nil(t, got)
}
...
func TestNewStdinReaderFromReader(t *testing.T) {
	...
	lines, err := r.FileDiff(FileDiffRequest{Path: "test.md"})
	require.NoError(t, err)
	require.Len(t, lines, 2)
	assert.Equal(t, "line one", lines[0].Content)
	assert.Equal(t, "line two", lines[1].Content)
...
func (s *stubInnerRenderer) FileDiff(req FileDiffRequest) ([]DiffLine, error) {
	s.gotContext = req.ContextLines
	return nil, nil
}
...
func TestFallbackRenderer_FileDiff_passesContextLinesThrough(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := fr.FileDiff(FileDiffRequest{Path: "main.go", ContextLines: tt.context})
			require.NoError(t, err)
