...
func TestHg_E2E_FullPipeline(t *testing.T) {
	...
	newLines, err := h.FileDiff(FileDiffRequest{Path: "new.txt"})
	require.NoError(t, err)
...
func TestHg_E2E_RefDiff(t *testing.T) {
	...
	lines, err := h.FileDiff(FileDiffRequest{Ref: "0..2", Path: "hello.txt"})
	require.NoError(t, err)
...
func TestGit_E2E_StillWorks(t *testing.T) {
	...
	lines, err := g.FileDiff(FileDiffRequest{Path: "hello.txt"})
	require.NoError(t, err)
	require.NotEmpty(t, lines)
